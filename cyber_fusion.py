...
def run_deep_asset_scan(url, recurse=False, concurrency=1):
    ...
    print(f"\n\033[94m\033[1m┌── DEEP ASSET AUDIT ───────────────────────────────────────────────┐\033[0m")
    print(f"│ \033[2mScanning for broken links and assets... (Safe mode active)\033[0m")
...
def print_master_report(url, intel_res, local_audit):
    ...
    print(f"\n{C_BLUE}{C_BOLD}╔" + "═" * 78 + "╗")
    print(f"║ {C_CYAN}OMNIPOTENT MASTER AUDIT {C_DIM}v4.0{C_END}{C_BLUE}{" " * (77 - 24 - 4)}║")
    print(f"║ {C_BOLD}TARGET:{C_END} {url}{" " * (78 - len(url) - 10)} ║")
    print(f"╚" + "═" * 78 + f"╝{C_END}")
    ...
    for c in displayed_cats:
        ...
        for r in sorted(cats[c], key=lambda x: (0 if x['score'] == 'bad' else 1 if x['score'] == 'warning' else 2)):
            icon = f"{C_GREEN}●{C_END}" if r['score'] == 'good' else f"{C_YELLOW}▲{C_END}" if r['score'] == 'warning' else f"{C_RED}✖{C_END}" if r['score'] == 'bad' else f"{C_DIM}○{C_END}"
            print(f"│ {icon} {C_BOLD}{r['name']}{C_END}")
            for line in r.get('rec', '').split('\n'): print(f"│   {C_DIM}└─{C_END} {line}")
            if r['name'] in STANDARDS: print(f"│      {C_DIM}💡 {STANDARDS[r['name']]}{C_END}")
